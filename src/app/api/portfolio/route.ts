...
export async function GET() {
  try {
    const { rows } = await query(
      "SELECT title, slug, excerpt, cover_url, published_at, tags FROM portfolio_posts WHERE status = $1 ORDER BY published_at DESC LIMIT 50",
      ["published"]
    );
...
export async function POST(req: Request) {
  ...
  try {
    const { rows } = await query<{ slug: string }>(
      `INSERT INTO portfolio_posts (title, slug, excerpt, cover_url, tags, content, status, published_at)
